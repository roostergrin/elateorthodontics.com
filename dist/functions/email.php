...
function rg_register_routes () {
  ...
  register_rest_route('rg-mail/v1', 'promotion', array(
    'methods' => WP_REST_Server::CREATABLE,
    'callback' => 'rg_serve_route_promotion'
  ));
  register_rest_route('rg-mail/v1', 'promotion-es', array(
    'methods' => WP_REST_Server::CREATABLE,
    'callback' => 'rg_serve_route_promotion_es'
  ));
}
...
function rg_serve_route_promotion () {
  require('wp-load.php');
...
function rg_serve_route_promotion_es () {
  require('wp-load.php');
  ...
  $from = 'info@wordpress.com';
  $to = 'info@elateorthodontics.com';
  $subject = 'Promotion Form - Spanish';
  $headers = array('Content-Type: text/html; charset=UTF-8');
  $message = '<html><body>';
  $message .= '<p><h4><strong>Form Submission by: </strong></h4>' . $data['fullName'] . '</p>';
  $message .= '<p><h4><strong>Email: </strong></h4>' . $data['clientEmail'] . '</p>';
